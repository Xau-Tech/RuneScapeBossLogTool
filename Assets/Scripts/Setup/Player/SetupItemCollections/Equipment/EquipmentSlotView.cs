...
public class EquipmentSlotView : AbsSetupItemSlotView, IPointerClickHandler, ITooltipHandler
{
    ...
    public override void Init(int equipmentSlotNumber)
    {
    ...
    public string GetTooltipMessage()
    {
        ...
        {
            tooltip += $"Total aug costs: {CacheManager.SetupTab.Setup.Player.AugmentsCost.ToString("N0")} gp/hr";
        }
        //  Otherwise simply get the value
        else
        {
            tooltip += $"Cost: {itemSlot.GetValue().ToString("N0")} gp/hr";
        }
