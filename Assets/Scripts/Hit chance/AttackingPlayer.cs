...
public class AttackingPlayer
{
    ...
    public int Accuracy()
    {
        return (int)(BaseAccuracy() * _accMod.Modifier);
    }
    ...
    {
        return (int)(Mathf.Pow(_combatLevel, 3) * .0008 + (4 * _combatLevel) + 40);
    }

    //  f(x) = x^3 / 500 + 10x + 100 always rounded down where x is the weapon accuracy tier
    private int WeaponAccuracy()
    {
        return (int)((Mathf.Pow(this.Weapon.AccuracyTier, 3) / 500.0f) + (10.0f * this.Weapon.AccuracyTier) + 100.0f);
    }
}
